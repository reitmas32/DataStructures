...
class Map{
	...
	public:
		Couple<ItemOne,ItemTwo> *table;		//La hice Publica para no hacer un getTable
		...
		bool Retrieve(ItemOne key, Couple<ItemOne,ItemTwo> *value);
		bool Remove(ItemOne key);
		bool Remove(ItemOne key, Couple<ItemOne,ItemTwo> *value);
...
template <typename ItemOne, typename ItemTwo>
bool Map<ItemOne,ItemTwo>::Retrieve(ItemOne key, Couple<ItemOne,ItemTwo> *value){
	int h=0;
	size_t start_loc = this -> hash(key,this -> size);
...
template <typename ItemOne, typename ItemTwo>
bool Map<ItemOne,ItemTwo>::Remove(ItemOne key, Couple<ItemOne,ItemTwo> *value){
	size_t loc = this -> hash(key,this -> size);
	size_t i = loc;
	int h= 0;
	bool done = false;
